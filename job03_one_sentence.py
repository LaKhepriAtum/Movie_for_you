...
for title in df['title'].unique():
    ...
    temp = temp['cleaned_sentences'] # 전처리 된 리뷰
    one_sentence = ' '.join(temp) # 하나의 문장으로 합친다. -> 리뷰단 하나
    one_sentences.append(one_sentence)
df_one_sentence = pd.DataFrame(
    {'titles':df['title'].unique(), 'cleaned_sentences':one_sentences})# df['title'].unique() 타이틀이 하나만
