...
key_word = '아이언맨' # keyword 를 기준으로 상대적인 좌표
sim_word = embedding_model.wv.most_similar(key_word, topn = 20)# keyword 근처에 있는 단어
print(sim_word)
...
for label, _ in sim_word: # sim_word 에서 2개의 값을 반환하는데 안 사용할 유사도는 _로 받는다.
    labels.append(label)
    vectors.append(embedding_model.wv[label]) #wv->word vector, label 하나하나의 vector 값
df_vectors = pd.DataFrame(vectors) # 100개 차원의 좌표값(벡터)
print(df_vectors.head())

# 차원의 저주 : 차원이 많아질수록 data 간의 거리(밀도)가 기하급수적으로 늘어난다-> data가 희소해 진다.
tsne_model = TSNE(perplexity=40, n_components=2, # TSNE 100 차원을 2차원으로
                  init='pca', n_iter=2500) # n_iter ->학습모델
...
# label 이 있으면 지도학습(답 맞추기), label 이 없으면 비지도학습(정답이 없다-> 시각화해서 직접 확인하는 수밖에 없다)